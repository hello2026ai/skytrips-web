...
// Function to determine refund status based on fare rules
const determineRefundStatus = (
  flight: any
): 'REFUNDABLE' | 'NON_REFUNDABLE' => {
  // Check if fareRules exist and find the refund rule
  const refundRule = flight?.fareRules?.rules?.find(
    (rule: any) => rule.category === 'REFUND'
  );

  // If there's no refundRule, it's refundable
  if (!refundRule) {
    return 'NON_REFUNDABLE';
  }

  // If there's a maxPenaltyAmount, it means it's refundable with penalty
  if (refundRule.maxPenaltyAmount) {
    return 'REFUNDABLE';
  }

  // If notApplicable is true, it's non-refundable
  // Otherwise (if notApplicable is false/undefined), it's refundable
  return refundRule.notApplicable ? 'NON_REFUNDABLE' : 'REFUNDABLE';
};
...
export default function Book() {
  ...
  const [showAirportTransfer, setShowAirportTransfer] = useState(false);

  // Debug showAirportTransfer changes
  useEffect(() => {
    console.log('showAirportTransfer state changed to:', showAirportTransfer);
  }, [showAirportTransfer]);
  const [selectedTransfers, setSelectedTransfers] = useState<{
    [key: string]: boolean;
  }>({});
  const [airportTransferAmounts, setAirportTransferAmounts] = useState<{
    [key: string]: number;
  }>({});
  const [convertedAirportTransferAmounts, setConvertedAirportTransferAmounts] =
  ...
  const [serviceFeeData, setServiceFeeData] = useState<any>([]); // Store all service fees
  const [filteredServiceFees, setFilteredServiceFees] = useState<any>([]); // Store filtered service fees
  const [bookingServiceFee, setBookingServiceFee] = useState<any>({
    amount: 0.0,
    currencyCode: 'AUD',
  });

  // Effect to check airport transfer conditions
  // Effect to check airport transfer conditions
  useEffect(() => {
    if (serviceFeeData && flightData) {
      console.log('test serviceFeeData', serviceFeeData);
      console.log('test flightData', flightData);
      // Find pick and drop fee from all service fees
      console.log('Checking conditions:', {
        originIata:
          flightData?.flight?.itineraries?.[0]?.segments?.[0]?.departure
            ?.iataCode,
        destIata:
      ...
      const pickAndDropFee = serviceFeeData.find(
        (fee: {
          propertyType: string;
          appliedType: string;
          ...
          fee.propertyType === 'AIRPORT_PICK_AND_DROP_FEE' &&
          fee.appliedType === 'SELECTED' &&
          // Check if origin airport matches
          fee.originAirport?.iataCode ===
            flightData?.flight?.itineraries?.[0]?.segments?.[0]?.departure
              ?.iataCode &&
          // Check if destination airport matches
          fee.destinationAirport?.iataCode ===
            flightData?.flight?.itineraries?.[0]?.segments?.slice(-1)[0]
              ?.arrival?.iataCode &&
          (fee.tripType === 'ALL' ||
            fee.tripType === flightData?.tripType?.toUpperCase() ||
            fee.tripType?.toLowerCase() === flightData?.tripType)
      );

      // Show pick and drop option if available
      // Log exact IATA codes for comparison
      const departureIata =
        flightData?.flight?.itineraries?.[0]?.segments?.[0]?.departure
          ?.iataCode;
      const arrivalIata =
      ...
      console.log('IATA Code Comparison:', {
        'Flight Departure IATA': departureIata,
        'Flight Arrival IATA': arrivalIata,
        'Service Fee Origin IATA': serviceFeeData.find(
          (f: { propertyType: string; originAirport?: { iataCode: string } }) =>
            f.propertyType === 'AIRPORT_PICK_AND_DROP_FEE'
        )?.originAirport?.iataCode,
        'Service Fee Destination IATA': serviceFeeData.find(
          (f: {
            propertyType: string;
            destinationAirport?: { iataCode: string };
          }) => f.propertyType === 'AIRPORT_PICK_AND_DROP_FEE'
      ...
      console.log('Found pickAndDropFee:', pickAndDropFee);
      console.log('Setting showAirportTransfer to:', !!pickAndDropFee);
      setShowAirportTransfer(!!pickAndDropFee);

      // Debug the matching conditions for each fee
      serviceFeeData.forEach(
        (
          fee: {
            propertyType: string;
            appliedType: string;
          ...
          console.log(`Fee ${index + 1} matching conditions:`, {
            propertyTypeMatch: fee.propertyType === 'AIRPORT_PICK_AND_DROP_FEE',
            appliedTypeMatch: fee.appliedType === 'SELECTED',
            originMatch:
              fee.originAirport?.iataCode ===
              flightData?.flight?.itineraries?.[0]?.segments?.[0]?.departure
                ?.iataCode,
      ...
      // Update airport transfer amounts if any are selected
      if (pickAndDropFee && pickAndDropFee._id && pickAndDropFee.amount) {
        setSelectedTransfers((prev) => ({
          ...prev,
          [pickAndDropFee._id]: true,
        }));
        setAirportTransferAmounts((prev) => ({
          ...prev,
          [pickAndDropFee._id]: Number(pickAndDropFee.amount || 0),
        }));
      }
    }
  }, [serviceFeeData, flightData]);

  console.log('airportTransferAmounts', airportTransferAmounts);
  ...
  // Listen for login events from navbar or other components
  useEffect(() => {
    const handleUserLogin = async (event: Event) => {
      const customEvent = event as CustomEvent<{ accessToken: string }>;
      console.log('Book page: Received userLoggedIn event', customEvent.detail);
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            'Book page: User data updated after navbar login',
            userData
          );
          // Also fetch relationships for the new user
          fetchRelationships();
        }
      } catch (error) {
        console.error(
          'Book page: Error fetching user data after login:',
          error
        );
      }
    };
    ...
    try {
      ...
      if (flightDataSource) {
        ...
        if (flightDataSource.flight) {
          // Make background API call to service fee for all fares
          axiosInstance
            .get('/service-fee')
            .then((response) => {
              console.log('Service fee API response:', response.data.data);
              if (response.data.data && response.data.data.length > 0) {
                // Store all service fees
                setServiceFeeData(response.data.data);

                // Filter and store matched fees separately
                const matchedFees = response.data.data.filter(
                  (fee: { appliedType: string; propertyType?: string }) =>
                    fee.appliedType === 'AUTOMATIC' &&
                    fee.propertyType !== 'AIRPORT_PICK_AND_DROP_FEE'
                );
                console.log('Matched service fees:', matchedFees);

                // Set filtered service fees
                setFilteredServiceFees(matchedFees);
              }
            })
            .catch((error) => {
          ...
          // Skip price API call for group fares
          if (!flightDataSource.flight.isGroupFare) {
            // Make background API call to flight price
            axiosInstance
              .post('/flight-price', flightDataSource.flight)
              .then((response) => {
                console.log(
                  'Flight price API response status:',
                  response.status
                );
              ...
              .then((data) => {
                ...
                if (
                  ...
                  if (filteredServiceFees[0]?.amount !== 0.0) {
                    toast.success(
                      `Flight price has been changed from ${
                        ...
                        Number(data.data[0].price.grandTotal) +
                          Number(filteredServiceFees[0]?.amount || 0)
                      ).toFixed(2)}`
                    );
                  } else {
                    toast.success(
  ...
  useEffect(() => {
    ...
    if (flightData?.flight && !bookingSuccess) {
      ...
      // Skip price check for group fares
      if (!flightData.flight.isGroupFare) {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // Collect service fee IDs
      const appliedServiceFeeIds: string[] = [];

      // Add automatic service fees
      if (filteredServiceFees.length > 0) {
        filteredServiceFees.forEach((fee: { _id?: string; id?: string }) => {
          if (fee._id || fee.id) {
            appliedServiceFeeIds.push(fee._id || (fee.id as string));
          }
        });
      }

      // Add selected airport pick and drop fees
      if (
        Object.values(selectedTransfers).some((isSelected) => isSelected) &&
        serviceFeeData
      ) {
        // Find all selected transfer fees
        Object.entries(selectedTransfers)
          .filter(([_, isSelected]) => isSelected)
          .forEach(([feeId]) => {
            const fee = serviceFeeData.find(
              (f: { _id?: string; id?: string }) =>
                f._id === feeId || f.id === feeId
            );
            if (fee && (fee._id || fee.id)) {
              appliedServiceFeeIds.push(fee._id || (fee.id as string));
            }
          });
      }

      // Prepare booking payload based on whether it's a group fare or not
      const bookingPayload = flightData?.flight?.isGroupFare
        ? {
            groupFareId: flightData.flight.groupFareId, // This should be provided in the flight data
            userInput: {
              ...userInput,
              id: userData?.id, // If user is logged in
            },
            passengerInput: passengerInputs.map((passenger) => ({
              ...passenger,
              confirmEmail: passenger.email,
              bookingId: '', // This will be generated by the backend
              bookingType: 'ONLINE',
              ticketNumber: '', // This will be generated by the backend
            })),
            travelClass:
              flightData?.flight?.travelerPricings?.[0]
            ...
            destination: additionalFields.destination || '',
            clientOrigin:
              additionalFields.clientOrigin || 'www.skytrips.com.au',
            isGroupFareApplied: true,
            bookingTicketRefundableStatus: determineRefundStatus(
              flightData?.flight
            ),
            appliedServiceFeeIds: appliedServiceFeeIds,
          }
        : {
      ...
      // Use different endpoint for group fares
      const bookingEndpoint = flightData?.flight?.isGroupFare
        ? '/flight/group-fare/booking'
        : '/flight-booking';
      ...
      // For group fares, use the bookingId from the response
      // For regular fares, use the id from the data property
      const bookingId = flightData?.flight?.isGroupFare
        ? bookingResult.bookingId
        : bookingResult.data?.id;

      if (!bookingId) {
        throw new Error('No booking ID received from the server');
      }

      bookingIdRef.current = bookingId;
      setBookingResponse(
        flightData?.flight?.isGroupFare ? bookingResult : bookingResult.data
      );
  ...
  const handlePaymentSubmit = async (bookingId?: string) => {
    try {
      ...
      // For group fares, use the ID as is
      const bookingReference = rawBookingReference;
      ...
      // Use different endpoint for group fares
      const paymentEndpoint = flightData?.flight?.isGroupFare
        ? '/flight/group-fare/booking/payment'
        : '/flight-payment/v2/bpoint';

      // Add isItineraryOverridden for group fares
      if (flightData?.flight?.isGroupFare) {
        paymentPayload.isItineraryOverridden = false;
      }
      ...
      if (response.status === 201) {
        ...
        //     id: bookingReference,
        //     associatedRecords: [{ reference: bookingReference }],
        //     flightOffers: [
        //       {
        //         ...flightData.flight,
        //         id: flightData.flight.id,
        //         source: flightData.flight.source || 'GDS',
        //         instantTicketingRequired:
        //           flightData.flight.instantTicketingRequired,
        //         nonHomogeneous: flightData.flight.nonHomogeneous,
        //         oneWay: flightData.flight.oneWay,
        //         lastTicketingDate: flightData.flight.lastTicketingDate,
        //         numberOfBookableSeats: flightData.flight.numberOfBookableSeats,
        //         itineraries: flightData.flight.itineraries,
        //         price: {
        //           currency: flightData.currencyCode,
        //           total: flightData.totalPrice,
        //           grandTotal: flightData.totalPrice,
        //           base: flightData.flight.price?.base,
        //         },
        //         pricingOptions: flightData.flight.pricingOptions,
        //         validatingAirlineCodes:
        //           flightData.flight.validatingAirlineCodes,
        //         travelerPricings: flightData.flight.travelerPricings,
        //       },
        //     ],
        //     travelers: flightData.travelers,
        //     contacts: flightData.contacts,
        //     dictionaries:
  ...
  useEffect(() => {
    if (bookingSuccess) {
      ...
      if (bookingResponse) {
        console.log('bookingResponse in useEffect', bookingResponse);

        // Only transform data for group fare bookings
        const confirmationData = bookingResponse.isGroupFareApplied
          ? {
              id: bookingResponse.bookingId || bookingResponse.manualBookingId,
              associatedRecords: [
                {
                  reference:
                    bookingResponse.passengerNameRecord ||
                    bookingResponse.manualPassengerNameRecord,
                  creationDate: bookingResponse.createdAt,
                  originSystemCode: 'GDS',
                },
              ],
              flightOffers: [
                {
                  ...bookingResponse.detail.flightOffers[0],
                  price: {
                    currency: bookingResponse.currencyCode,
                    total: bookingResponse.totalPrice,
                    base: bookingResponse.detail.flightOffers[0].price.base,
                    grandTotal: bookingResponse.totalPrice,
                  },
                },
              ],
              travelers: bookingResponse.detail.travelers,
              contacts: bookingResponse.detail.contacts,
              dictionaries: bookingResponse.detail.dictionaries,
            }
          : bookingResponse;
  ...
  // Add a function to convert any amount using passed data
  const convertAmountWithData = async ({
    fromCurrency,
    toCurrency,
    amount,
    onSuccess,
    ...
    toCurrency: string;
    amount: number;
    onSuccess: (convertedAmount: number) => void;
  }) => {
    try {
      ...
      if (response?.data?.convertedAmount) {
        onSuccess(response.data.convertedAmount);
      } else {
        console.warn('Currency conversion failed, using original amount');
        onSuccess(amount); // fallback
      }
    } catch (error) {
  ...
  // Handle currency conversion for service fee and airport transfer
  useEffect(() => {
    if (!serviceFeeData || !flightData?.currencyCode) return;

    const targetCurrency = flightData.currencyCode;

    // Convert service fees
    const convertServiceFees = async () => {
      if (filteredServiceFees[0]?.amount) {
        const sourceCurrency = filteredServiceFees[0]?.currencyCode || 'AUD';
        const amount = Number(filteredServiceFees[0]?.amount || 0);

        console.log('Converting service fee:', {
          from: sourceCurrency,
          to: targetCurrency,
          amount: amount,
        });

        await convertAmountWithData({
          fromCurrency: sourceCurrency,
          toCurrency: targetCurrency,
          amount: amount,
          onSuccess: (convertedAmount) => {
            console.log('Service fee converted:', convertedAmount);
            setConvertedServiceFee(convertedAmount);
          },
        });
      } else {
        setConvertedServiceFee(0);
      }
    };

    // Convert airport transfer fees
    const convertAirportTransfers = async () => {
      const selectedFees = Object.entries(selectedTransfers)
        .filter(([_, isSelected]) => isSelected)
        .map(([feeId]) => {
          const fee = serviceFeeData.find((f: any) => f._id === feeId);
          return {
            feeId,
            amount: airportTransferAmounts[feeId] || 0,
            sourceCurrency: fee?.currencyCode || 'AUD',
          };
        });

      // Convert each selected fee
      for (const fee of selectedFees) {
        if (fee.amount > 0) {
          console.log('Converting airport transfer fee:', {
            from: fee.sourceCurrency,
            to: targetCurrency,
            amount: fee.amount,
          });

          await convertAmountWithData({
            fromCurrency: fee.sourceCurrency,
            toCurrency: targetCurrency,
            amount: fee.amount,
            onSuccess: (convertedAmount) => {
              console.log('Airport transfer fee converted:', {
                feeId: fee.feeId,
                amount: convertedAmount,
              });
              setConvertedAirportTransferAmounts((prev) => ({
                ...prev,
                [fee.feeId]: convertedAmount,
              }));
            },
          });
        }
      ...
      // Clear any unselected fees
      setConvertedAirportTransferAmounts((prev) => {
        const newAmounts = { ...prev };
        Object.keys(newAmounts).forEach((feeId) => {
          if (!selectedTransfers[feeId]) {
    ...
    convertServiceFees();
    convertAirportTransfers();
  }, [
    serviceFeeData,
    flightData?.currencyCode,
    filteredServiceFees,
    selectedTransfers,
    airportTransferAmounts,
  ]);
        ...
        {!bookingSuccess && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2 ">
                    ...
                    <div className="flex justify-between items-center mb-5 bg-white rounded-md px-4 py-2">
                      <div>
                        <h2 className="h5 text-background-on pb-1">
                          {(() => {
                            // Get departure from first itinerary's first segment (e.g., SYD)
                            const firstDeparture =
                              flightData.flight.itineraries[0]?.segments[0]
                                ?.departure?.iataCode || 'Origin';

                            // Get destination from first itinerary's last segment (e.g., KTM)
                            ...
                            // Format as SYD (Sydney) ↔ KTM (Kathmandu)
              ...
              {showAirportTransfer ? (
                <div className="bg-container rounded-md mb-5 shadow-sm">
                    ...
                    {serviceFeeData
                      .filter(
                        (fee: {
                          ...
                          fee.propertyType === 'AIRPORT_PICK_AND_DROP_FEE' &&
                          fee.appliedType === 'SELECTED' &&
                          // For outbound flight (first itinerary)
                          ((fee.originAirport?.iataCode ===
                            flightData?.flight?.itineraries?.[0]?.segments?.[0]
                              ?.departure?.iataCode &&
                            fee.destinationAirport?.iataCode ===
                              flightData?.flight?.itineraries?.[0]?.segments?.[
                                flightData?.flight?.itineraries?.[0]?.segments
                                  ?.length - 1
                              ]?.arrival?.iataCode) ||
                            // For return flight (second itinerary)
                            (flightData?.flight?.itineraries?.[1] && // Only check return if it exists
                              fee.originAirport?.iataCode ===
                                flightData?.flight?.itineraries?.[1]
                                  ?.segments?.[0]?.departure?.iataCode &&
                              fee.destinationAirport?.iataCode ===
                                flightData?.flight?.itineraries?.[1]
                                  ?.segments?.[
                                  flightData?.flight?.itineraries?.[1]?.segments
                                    ?.length - 1
                                ]?.arrival?.iataCode))
                      )
                      .map(
                        (
                          ...
                          <div
                            key={fee.id || `transfer-${index}`}
                            className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg mb-2"
                          >
                            <input
                              type="checkbox"
                              id={`airport-transfer-${fee.id}`}
                              className="mt-1"
                              checked={selectedTransfers[fee.id] || false}
                              onChange={(e) => {
                                const isChecked = e.target.checked;

                                // Simply update this specific fee's selection
                                setSelectedTransfers((prev) => ({
                                  ...prev,
                                  [fee.id]: isChecked,
                                }));

                                // Update the amount for this specific fee
                                setAirportTransferAmounts((prev) => ({
                                  ...prev,
                                  [fee.id]: isChecked
                                    ? Number(fee.amount || 0)
                            ...
                            <div className="flex-grow">
                              <label
                                htmlFor={`airport-transfer-${fee.id}`}
                                className="flex justify-between"
                              >
                                <div>
                                  <h4 className="title-t4">
                                    {fee.name ||
                                      (fee.destinationAirport?.iataCode
                                        ? `Pick and Drop - ${fee.originAirport?.iataCode} to ${fee.destinationAirport?.iataCode}`
                                        : 'Pick and Drop Service')}
                                  </h4>
                                  <span className="label-l3 text-neutral-dark">
                                    {fee.description ||
                                      'Comfortable private car service'}
                                    {fee.tripType &&
                                      fee.tripType !== 'ALL' &&
                                      ` (${fee.tripType.replace('_', ' ')})`}
                                  </span>
                                </div>
                                <span className="title-t4">
                                  {flightData?.currencyCode || 'AUD'}{' '}
            ...
            <div className="lg:col-span-1 sticky top-5 self-start mb-3">
              <div className="bg-white rounded-md p-5 shadow-sm">
                ...
                <div className="mb-5">
                  ...
                  {/* For group fares, show only total price without any fees */}
                  {/* Calculate matching fee */}
                  {(() => {
                    // For round trips, we want origin -> destination (e.g., SYD -> KTM)
                    const firstDeparture =
                      flightData?.flight?.itineraries?.[0]?.segments?.[0]
                        ?.departure?.iataCode;
                    const firstDestination =
                      flightData?.flight?.itineraries?.[0]?.segments?.[
                        flightData?.flight?.itineraries?.[0]?.segments?.length -
                          1
                      ]?.arrival?.iataCode;
                    ...
                    const matchingFee = serviceFeeData?.find(
                      (fee: {
                        propertyType: string;
                        appliedType: string;
                        originAirport?: { iataCode: string };
                        ...
                        fee.propertyType === 'AIRPORT_PICK_AND_DROP_FEE' &&
                        fee.appliedType === 'SELECTED' &&
                        fee.originAirport?.iataCode === firstDeparture &&
                        fee.destinationAirport?.iataCode === firstDestination &&
                        (fee.tripType === 'ALL' ||
                          fee.tripType ===
                            flightData?.tripType?.toUpperCase() ||
                          fee.tripType?.toLowerCase() === flightData?.tripType)
                    );

                    console.log('Service Fee Match:', {
                      matchingFee,
                      serviceFees: serviceFeeData?.map(
                        (fee: {
                          propertyType: string;
                          appliedType: string;
                          originAirport?: { iataCode: string };
                          ...
                          propertyType: fee.propertyType,
                          appliedType: fee.appliedType,
                          origin: fee.originAirport?.iataCode,
                          destination: fee.destinationAirport?.iataCode,
                        ...
                        <div>
                          ...
                          <div className="flex justify-between label-l3 text-background-on mb-2">
                            ...
                            <span>
                              {flightData?.currencyCode || 'AUD'}{' '}
                              {Number(
                                flightData?.flight?.price?.grandTotal || 0
                              ).toFixed(2)}
                            </span>
                          </div>
                            ...
                            Object.entries(selectedTransfers)
                              ...
                              .map(([feeId]) => {
                                ...
                                return (
                                  <div
                                    key={feeId}
                                    className="flex justify-between label-l3 mb-4"
                                  >
                                    <span className="label-l3 text-background-on">
                                      {fee.name ||
                                        `${fee.originAirport?.iataCode} to ${fee.destinationAirport?.iataCode} Transfer`}
                                    </span>
                                    <span className="label-l3 text-background-on">
                                      {flightData?.currencyCode || 'AUD'}{' '}
                                      {Number(
                                        convertedAirportTransferAmounts[
                                          feeId
                                        ] || 0
                                      ).toFixed(2)}
                                    </span>
                              ...
                              <span className="h5 text-primary">
                                ...
                                {Number(
                                  Number(
                                    ...
                                    Number(convertedServiceFee || 0) +
                                    (Object.values(selectedTransfers).some(
                    ...
                    } else if (flightData?.flight?.travelerPricings) {
                      return (
                        <div>
                          {(() => {
                            const renderTravelerGroups =
                              (): React.ReactNode => {
                                ...
                                interface TravelerGroups {
                                  [key: string]: TravelerGroup;
                                }

                                // Group travelers by type and sum their prices
                                ...
                                // Convert entries to array and type-cast
                                const entries = Object.entries(
                                  travelerGroups
                                ) as [string, TravelerGroup][];
                  ...
                  {/* Show taxes and fees only for non-group fares */}
                  {!flightData?.flight?.isGroupFare && (
                    <>
                      <div className="flex justify-between label-l3 text-background-on mb-2">
                        <div
                        ...
                        <span>
                          ...
                          {(() => {
                            ...
                            const serviceFee = Number(
                              filteredServiceFees[0]?.amount || 0
                            );
                            const pickAndDropFees = Object.values(
                              selectedTransfers
                            ...
                            Object.entries(selectedTransfers)
                              ...
                              .map(([feeId]) => {
                                ...
                                const convertedAmount =
                                  convertedAirportTransferAmounts[feeId] || 0;

                                console.log('fee in price summary', fee);
                                return (
                                  <div
                                    key={feeId}
                                    className="flex justify-between label-l3 mt-2"
                                  >
                                    <span className="label-l3 text-background-on">
                                      {fee?.name ||
                                        (fee?.destinationAirport?.iataCode
                  ...
                  <div className="pt-4 border-t border-gray-200">
                    <div className="flex justify-between items-center font-medium">
                      ...
                      <span className="h5 text-primary">
                        {flightData?.currencyCode || 'AUD'}{' '}
                        {(() => {
                          // Calculate base price (Adult x 1)
                          const basePrice = Number(
                            flightData?.flight?.price?.base || 0.0
                          ); // 2753.00
                          ...
                          const serviceFee = Number(convertedServiceFee || 0); // 20.00 (converted)
                          const pickAndDropFees = Object.entries(
                            selectedTransfers
                          )
                            .filter(([_, isSelected]) => isSelected)
                            .reduce(
                              (total, [feeId]) =>
                                ...
                                (convertedAirportTransferAmounts[feeId] || 0),
                              0
                            ); // Sum of all selected transfers

                          // Total with all components
                          const totalWithPickAndDrop =
                            basePrice + taxes + serviceFee + pickAndDropFees; // 2753.00 + 562.68 + 20.00 + 30.00 = 3365.68

                          const discountAmount = fareDiscount
                            ? fareDiscount.amount
                            : 0;
                          const finalTotal =
                            totalWithPickAndDrop - discountAmount;
